...
function initHeartRain() {
    ...
    function animateHearts() {
        ...
        hearts.forEach(heart => {
            ...
            // If the cloud goes off-screen to the right, reset it to the left
            if (heart.x > canvas.width + heart.size) {
                heart.x = -heart.size;
                heart.y = Math.random() * canvas.height;
            }
            
            // Dibujar nube without rotation
            drawCloud(ctx, heart.x, heart.y, heart.size, heart.color, 0);
        });
...
// Dibujar nube
function drawCloud(ctx, x, y, size, color, rotation) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotation);
    ctx.fillStyle = color;

    // A cloud is drawn as a series of overlapping circles without a stroke.
    const positions = [
        { dx: 0, dy: 0, r: 0.60 },
        { dx: -0.6, dy: 0.1, r: 0.40 },
        { dx: 0.6, dy: 0.1, r: 0.50 },
        { dx: -0.2, dy: -0.4, r: 0.50 },
        { dx: 0.3, dy: -0.5, r: 0.45 }
    ];

    positions.forEach(p => {
        ctx.beginPath();
        ctx.arc(p.dx * size, p.dy * size, p.r * size, 0, 2 * Math.PI);
        ctx.fill();
    });
...
// Obtener color aleatorio de nube
function getRandomCloudColor() {
    const colors = ['#f0f8ff', '#f0f8ff', '#e6e6fa', '#f0f8ff', '#f3f3f3'];
